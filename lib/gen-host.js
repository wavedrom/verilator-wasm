...
const {parseTuList, EXPECTED_COUNT} = require('./tu-list.js');
...
const collectGenerated = async (forkRoot, files) => {
  ...
  if (generated.length < 19) {
    throw new Error('gen-host: only ' + generated.length + ' astgen headers in ' + objOpt + ', expected 19+');
  }
...
const genHost = async (opts = {}) => {
  ...
  const files = {};
  await collectGenerated(fork.root, files);
  await emitVrootData();

  const tus = await parseTuList(fork.root);
  if (tus.length !== EXPECTED_COUNT) {
    throw new Error('gen-host: parsed ' + tus.length + ' TUs, expected ' + EXPECTED_COUNT);
  }
